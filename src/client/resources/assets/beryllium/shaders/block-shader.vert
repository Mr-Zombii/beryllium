...
int UV_ROTATION = int(a_packed_2 & 0x3u);
...
vec4 getRotatedUV(vec4 uvs) {
    switch (UV_ROTATION) {
        case 1: return uvs.wxyz;
        case 2: return uvs.zwxy;
        case 3: return uvs.yzwx;
        default: return uvs;
    }
}
...
vec2 getAlbedoUV(void) {
    vec4 fetchedUVs = texelFetch(u_albedoUVBuffer, int(a_albedoIdx));
    vec4 uvs = getRotatedUV(fetchedUVs);
